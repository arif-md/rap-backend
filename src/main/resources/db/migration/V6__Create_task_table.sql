...
IF NOT EXISTS (SELECT 1 FROM sys.tables t JOIN sys.schemas s ON t.schema_id = s.schema_id WHERE s.name = 'RAP' AND t.name = 'task')
BEGIN
    ...
    PRINT 'Created table: RAP.task';
END
ELSE
  PRINT 'Table RAP.task already exists';
GO
